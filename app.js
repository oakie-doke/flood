...
class FloodRiskApp {
    ...
    async assessLocationRisk(lat, lng) {
        try {
            ...
            const riskData = this.calculateFloodRisk(elevationData.elevation, rainfall);
    ...
    calculateFloodRisk(elevation, rainfall) {
        ...
        // 1. ELEVATION FACTOR (Weight: 20%)
        // Lower elevation increases flood risk
        if (elevation < 10) {
            riskFactors.elevation = 10; // Very high risk - coastal/low-lying areas
        } else if (elevation < 30) {
            riskFactors.elevation = 8;  // High risk - flood plains
        } else if (elevation < 50) {
            riskFactors.elevation = 6;  // Medium-high risk
        } else if (elevation < 100) {
            riskFactors.elevation = 4;  // Medium risk
        } else if (elevation < 200) {
            riskFactors.elevation = 2;  // Low-medium risk
        } else {
            riskFactors.elevation = 0;  // Very low risk - high elevation
        }
        
        // 2. RAINFALL FACTOR (Weight: 80%)
        // Recent and forecasted rainfall intensity - primary risk factor
        if (rainfall > 50) {
            riskFactors.rainfall = 10; // Very high risk - extreme rainfall
        } else if (rainfall > 30) {
            riskFactors.rainfall = 8;  // High risk - heavy rainfall
        } else if (rainfall > 20) {
            riskFactors.rainfall = 6;  // Medium-high risk
        } else if (rainfall > 10) {
            riskFactors.rainfall = 4;  // Medium risk
        } else if (rainfall > 5) {
            riskFactors.rainfall = 2;  // Low-medium risk
        } else {
        ...
        console.log(`🔍 DEBUG: Risk calculation - Elevation: ${riskFactors.elevation} (weight: 20%), Rainfall: ${riskFactors.rainfall} (weight: 80%), Total Score: ${totalRiskScore.toFixed(2)}`);
        
        // Determine risk level based on total score
        let riskLevel;
        if (totalRiskScore >= 10) riskLevel = 'very-high';
        else if (totalRiskScore >= 6) riskLevel = 'high';
        else if (totalRiskScore >= 2) riskLevel = 'medium';
        else riskLevel = 'low';
     ...
     * Searches in 10-mile increments in all directions
     */
    async searchForSaferCity() {
        if (!this.userLocation) return;
        
        console.log('🔍 DEBUG: Searching for safer city...');
        ...
        // Search in 10-mile increments up to 100 miles
            ...
            for (const direction of directions) {
                ...
                try {
                    ...
                    if (riskData.level === 'low' || riskData.level === 'medium') {
                        ...
                        // Display the safer city
                        this.displaySaferCity(cityName, distance, direction.name, riskData.level);
                        return;
                    }
                } catch (error) {
        ...
        console.log('❌ DEBUG: No safer city found within 100 miles');
        this.hideSaferCityDisplay();
    }
     ...
     */
    displaySaferCity(cityName, distance, direction, riskLevel) {
